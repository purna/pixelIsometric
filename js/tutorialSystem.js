...
class TutorialSystem {
    ...
    init() {
        ...
        this._buildDOM();
        this._bindEvents();
        // Tutorial settings are handled by the main settings modal now
    ...
    startTutorial(tutorialId = 'main') {
        // Respect the tutorial enabled setting from the main settings modal
        if (window.stateManager) {
            const tutorialEnabled = window.stateManager.getStateProperty('tutorial.enabled') !== false;
            if (!tutorialEnabled) {
        ...
        const tryInject = (attempts = 0) => {
            ...
            wrap.innerHTML = `
                ...
                <div class="control-group checkbox-group" style="margin-bottom:10px">
                    <input type="checkbox" id="enableTutorialsSettings">
            ...
            const checkbox = document.getElementById('enableTutorialsSettings');
            // Set initial value from app state
            if (window.stateManager) {
                checkbox.checked = window.stateManager.getStateProperty('tutorial.enabled') !== false;
            } else {
                checkbox.checked = false; // default to off
            }

            checkbox.addEventListener('change', (e) => {
                // Update both tutorial config and app state
                this.config.tutorials.main.enabled = e.target.checked;
                if (window.stateManager) {
                    window.stateManager.setStateProperty('tutorial.enabled', e.target.checked);
                }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-start tutorial only if enabled in settings and first visit
    const seen = localStorage.getItem('pkb-tutorial-seen');
    if (!seen) {
        // Check tutorial enabled state from app settings (will be set by app.js)
        setTimeout(() => {
            // The app will handle tutorial auto-start based on settings
        }, 800);
    }
});
