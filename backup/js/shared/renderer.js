...
class Canvas2DRenderer {
    constructor(masterCanvas) {
        ...
        // Exposed shortcut (filled in by 2D app with state-derived color)
    ...
    // ─── Background color (shared from 3D state) ─────────────────────────────
    _applyBgFromState() {
        try {
            const st = window.stateManager?.getStateProperty?.('scene.backgroundColor')
                    || this._explicitBg;          // fallback: colour set directly by caller
            if (st) this._bgColor = st;
        } catch (_) {}
    }

    /**
     * Called by the 3D app before a 2D mode switch to set the current
     * background colour explicitly, avoiding the window.stateManager gap.
     */
    _setExplicitBg(color) {
        if (color) {
            this._explicitBg = color;
            this._bgColor = color;
        }
    }
