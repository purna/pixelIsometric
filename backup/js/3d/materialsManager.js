...
class MaterialsManager extends MaterialsManagerBase {
    constructor(app) {
        ...
        this._selectedObjectsGetter = null;
    }

    _getSelectedObjects() {
        return this._selectedObjectsGetter ? this._selectedObjectsGetter() : [];
    }

    setSelectedObjectsGetter(fn) {
        this._selectedObjectsGetter = fn;
    }
    ...
    _applyToObj3D(obj, mat) {
        ...
        if (mat.isPixelTexture) {
            const newMat = textureManager.getMaterial(mat.textureType);
            if (newMat) {
                obj.material = newMat;
